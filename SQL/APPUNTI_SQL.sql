    ...
    */
    ...
    NOT NULL /*É IMPLICATO SE L'ATTRIBUTO FA PARTE DI UNA CHIAVE PRIMARIA*/
    ...
    /*CREAZIONE VIEW*/
    CREATE VIEW (NOME) ["("ATTRIBUTO{,ATTRIBUTO}")"] AS QUERY-SELECT
    /*EX*/CREATE VIEW MediaVoti (Matricola,Media) AS SELECT Matricola, AVG(Voto) FROM Esami GROUP BY Matricola

    /*UTILIZZO DELLE VIEW*/
    SELECT * FROM (VIEW)
    /*EX*/ SELECT * FROM MediaVoti

    /*USO DELLE VIEW PER QUERY COMPLESSE*/
      /*-SEMPLIFICARE LE QUERY COMPLESSE*/
    /*EX*/CREATE VIEW AgPerZona (Zona,NumAg) AS SELECT Zona,COUNT(*) FROM AGENTI GROUP BY Zona
          SELECT AVG(NumAg) FROM AgPerZona 
          SELECT AVG(NumAg) FROM (SELECT Zona,COUNT(*) as NumAg FROM AGENTI GROUP BY Zona)

    /*VIEW USATA COME TABELLA*/
     /*-UNA VIEW PUÓ ESSERE DEFINITA SULLA BASE DI UN'ALTRA VIEW*/
     /*-COSA SUCCEDE SE LA TABELLA/VIEW USATA IN VIEW VIENE ELIMINATA:*/
        /*-LA VIEW VIENE INDICATA COME INOPERABILE*/
        /*-LA MODIFICA/CANCELLAZIONE VIENE NEGATA*/

    /*LIMITI ALGEBRA RELAZIONALE*/ /*VEDERE 10.PDF*/
